...
// return (
// <div className="group bg-zinc-900 col-span relative h-[12vw]" onMouseEnter={handleMouseEnter}
//     onMouseLeave={handleMouseLeave}>
//     <img src={`https://image.tmdb.org/t/p/w500${data.backdrop_path}`} alt="Movie" draggable={false}
//         className=" cursor-pointer object-cover transition duration shadow-xl rounded-md delay-300 w-full h-[12vw]" />
//     <div
//         className=" opacity-0 absolute top-0 transition duration-200 z-10 invisible sm:visible delay-300 w-full scale-0 group-hover:scale-110 group-hover:-translate-y-[6vw] group-hover:opacity-100">
//         <img src={`https://image.tmdb.org/t/p/w500${data.backdrop_path}`} alt="Movie" draggable={false}
//             className=" cursor-pointer object-cover transition duration shadow-xl rounded-t-md w-full h-[12vw]" />
//         {playVideo && (
//         <video ref={videoRef} autoPlay muted loop className="w-full h-full object-cover absolute top-0 left-0"
...
//                 <AiOutlineCheck className="fill-white w-10 h-10 bg-zinc-700 rounded-full border-2" onMouseEnter={()=>
//                     setShowAddToList(true)}
//                     onMouseLeave={() => setShowAddToList(false)}
//                     />
//                     ) : (
//                     <AiOutlinePlus className="fill-white w-10 h-10 bg-zinc-700 rounded-full border-2" onMouseEnter={()=>
//                         setShowAddToList(true)}
//                         onMouseLeave={() => setShowAddToList(false)}
//                         onClick={() => {
...
//                         {showAddToList && (
//                         <div
//                             className="absolute mb-24  transform px-3 py-2 bg-zinc-200 rounded-md text-black text-xl font-semibold whitespace-nowrap">
//                             {myList.some((item) => item.id === data.id)
//                             ? "Added"
//                             : "Add to My List"}
...
//                     {data.adult === "false" ? "16+" : "18+"}
//                 </div>
//             </div>
//             <p className="text-white font-light">
//                 {getGenreText(data.genre_ids)}
//             </p>
...
const MovieCard = ({ data, myList, addToMyList }) => {
  ...
  useEffect(() => {
    const fetchVideoId = async () => {
      try {
        const apiKey = "AIzaSyDgF9v-JZtD7tA5yZ1bkqultzpgKUCl8mI";
        const query = encodeURIComponent(`${data.title} official trailer`);
        const endpoint = `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${query}&type=video&key=${apiKey}`;
        const response = await axios.get(endpoint);
      ...
      <div className="opacity-0 absolute top-0 transition duration-200 z-10 invisible sm:visible delay-300 w-full scale-0 group-hover:scale-110 group-hover:-translate-y-[6vw] group-hover:opacity-100">
        ...
        <div className="z-10 bg-zinc-800 p-2 lg:p-4 absolute w-full transition shadow-md rounded-b-md ">
          <div className="flex flex-row items-center gap-3">
            ...
            {myList.some((item) => item.id === data.id) ? (
              <AiOutlineCheck
                className="fill-white w-10 h-10 bg-zinc-700 rounded-full border-2"
            ...
            {showAddToList && (
              <div className="absolute mb-24 transform px-3 py-2 bg-zinc-200 rounded-md text-black text-xl font-semibold whitespace-nowrap">
                {myList.some((item) => item.id === data.id) ? "Added" : "Add to My List"}
              </div>
            )}
          </div>
          <div className="flex items-center mt-4 mb-4 gap-2">
