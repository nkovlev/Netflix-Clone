  ...
  <div className="bg-black w-full h-full lg:bg-opacity-50 ">
    ...
    <div className="flex flex-col justify-center items-center h-screen">
      ...
      <div className="flex items-center px-6 py-6 gap-3">
        <Formik initialValues={{ useremail: '' }} validationSchema={Yup.object().shape({ useremail: Yup.string() .required('Required') .matches(/^[a-zA-Z0-9._-]+@[a-zA-Z0-9.-]+\.[a-zA-Z]/, 'Invalid email' ) })}>
          {({ errors, touched, handleChange, isValid }) => (
          <Form className='flex gap-3'>
            <div className="relative mx-auto w-full max-w-md">
              <Field placeholder="Enter your Email" type="text" name="useremail" id="useremail" value={email} onChange={(e)=> {
                handleChange(e)
                setEmail(e.target.value)
                }}
                className={`box-border w-full py-4 pr-40 pl-5 pt-4 h-14 outline-none bg-transparent ring-2 ${errors.useremail && touched.useremail ? 'ring-red-500' : 'ring-gray-400'} text-white font-semibold placeholder-gray-400 focus:placeholder-transparent focus:ring-2 focus:ring-white focus:border-transparent rounded`}
                />
                {errors.useremail && touched.useremail && (
                <div className="text-red-500 flex">{errors.useremail}</div>
                )}
            </div>
